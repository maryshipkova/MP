...
namespace OntologyMain.Api.Services
{
  ...
  public class StateContext
  {
    ...
    public static StateContext FromState(StateType type)
    {
      ...
      if (type == StateType.State3) return new StateContext {Description = "Предложить госпитализацию"};
      ...
      if (type == StateType.State5) return new StateContext {Description = "Выписка пациента"};
      ...
      if (type == StateType.State8) return new StateContext {Description = "Госпитализация."};
      ...
      if (type == StateType.State10) return new StateContext {Description = "Перевод в ОРИТ."};
      if (type == StateType.End) return new StateContext {Description = "Конечное состояние"};
